...
export async function listJourneys(db: D1Database): Promise<Journey[]> {
  const { results } = await db
    .prepare(`SELECT ${JOURNEY_COLUMNS} FROM journeys ORDER BY start_date DESC, id DESC`)
    .all<JourneyRow>();
...
export async function getJourneyBySlug(db: D1Database, slug: string): Promise<Journey | null> {
  const row = await db
    .prepare(`SELECT ${JOURNEY_COLUMNS} FROM journeys WHERE slug = ?1`)
    .bind(slug)
    .first<JourneyRow>();
...
export async function createJourney(db: D1Database, input: JourneyInput): Promise<Journey> {
  ...
  try {
    const row = await db
      .prepare(`
        INSERT INTO journeys (slug, name, name_zh, start_date, end_date, description, cover)
...
export async function deleteJourney(db: D1Database, id: number): Promise<number> {
  const visits = await db
    .prepare("SELECT COUNT(*) AS count FROM visits WHERE journey_id = ?1")
    .bind(id)
    .first<{ count: number }>();
  const result = await db.prepare("DELETE FROM journeys WHERE id = ?1").bind(id).run();
